"""Fake NVM driver, for testing purposes.

Memory is very much volatile, and is stored in RAM
"""
NVM_SIZE = 1000
...
class bitFlag:
    ...
    def __get__(self, obj, objtype=None):
        return bool(nvm[self.byte] & self.bit_mask)

    def __set__(self, obj, value):
        if value:
            nvm[self.byte] |= self.bit_mask
        else:
            nvm[self.byte] &= ~self.bit_mask
...
class multiBitFlag:
    ...
    def __get__(self, obj, objtype=None):
        return (nvm[self.byte] & self.bit_mask) >> self.lowest_bit
...
class multiByte:
    ...
    def __get__(self, obj, objtype=None):
        return int.from_bytes(nvm[self.start:self.stop], 'big')

    def __set__(self, obj, value):
        if value >= self.maxval:
            value = self.maxval
        nvm[self.start:self.stop] = value.to_bytes(self.num_bytes, 'big')
